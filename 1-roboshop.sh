...
do
  INSTANCE_ID=$(aws ec2 run-instances --image-id ami-09c813fb71547fc4f --instance-type t2.micro --security-group-ids sg-01fb8f67c6dc9cd19 --tag-specifications "ResourceType=instance,Tags=[key,value]" --query "Instances[0].InstanceId" --output text) 
